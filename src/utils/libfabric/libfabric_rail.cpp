...
nixlLibfabricRail::nixlLibfabricRail(const std::string &device,
    ...
    if (provider == "tcp" || provider == "sockets") {
        ...
        // EFA, CXI and other providers support advanced memory registration
...
bool
nixlLibfabricRail::supportsVirtAddr() const {
    return (info && info->domain_attr && (info->domain_attr->mr_mode & FI_MR_VIRT_ADDR));
}
